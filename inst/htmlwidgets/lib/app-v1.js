...
(function(scope) {
  ...
  scope.chord = function(arrowRatio) {
    ...
    function chord(d, i) {
      ...
      var ccp = cubic_control_points(s, t, s.r * curveFactor);
...
(function(scope) {
  scope.timeline = function(diagram, config) {
    var years = Object.keys(diagram.data.matrix).map(function(y) { return parseInt(y); });  // selects the Matrices Data("YEARS") from the MATRIX

    // when config is null then, use the other configurations given as OR !!!
...
var select = d3.select(config.element)
  .append('select')
  	.attr('class','select')
    .on('change',onchange)
...
function onchange() {
	selectValue = d3.select('select').property('value')
		...
		diagram.draw(selectValue) ;
};

	// END of the new Dropdown list ::::
...
(function(scope) {
  ...
  scope.chart = function(data, config) {
    ...
    data = data || { regions: [], names: [],name1 : [], name2 :[], reg_col : [] , matrix: [] };
   ...
   // config.arcPadding = config.arcPadding || 5;
...
curveFactor = config.curveFactor ;
   ...
   // config.maxRegionsOpen = config.maxRegionsOpen;
   // config.infoPopupDelay = config.infoPopupDelay || 300;

    // config.reg_col = config.reg_col ;
    ...
    // chord path generator
      //
    var chordGenerator = Globalmigration.chord(0.05) // how sharp the pointed edges should be
    // more the value, more pointed...
        .radius(config.innerRadius )
        .sourcePadding(config.sourcePadding - 10) // can increase/decrease the spaces
        .targetPadding(config.targetPadding - 10);  // between the chords and arcs here

    // svg element
    // ...can set the background color here
...
// TODO :: FIX CIRCLE OUT
    ...
    function draw(year, countries) {
      year = year || Object.keys(data.matrix)[1];
